...
public class AAAPermissionEvaluator implements PermissionEvaluator {
    ...
    private boolean hasPrivilege(Authentication auth, String targetType, String permission) {
        ...
        // Check if permission is in user operations.
        boolean userHasOperation = user.getUserHasOperations().stream()
            .anyMatch(uho -> uho.getOperation().getName().toUpperCase().equals(permission.toUpperCase()) && !uho.isDeny());

        // If user has no such operation, check in group operations.
        return userHasOperation || user.getGroupHasOperations().stream()
            .anyMatch(gho -> gho.getOperation().getName().toUpperCase().equals(permission.toUpperCase()) && !gho.isDeny());
    }
