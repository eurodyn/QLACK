...
@Service
@Validated
public class QFALdapUserUtil {

  private static final Logger LOGGER = Logger.getLogger(QFALdapUserUtil.class.getName());
...
//		user.setSalt("external");
  ...
  private String addGroupFromLdap(QFAUser user, Map<String, List<String>> ldap) {
    String groupId = getFirst(ldap, ldapMappingGid);
    if (groupId == null) {
      return null;
    }

    QFAGroup group = em.find(QFAGroup.class, groupId);
    if (group == null) {
      return null;
    }

    List<QFAUser> users = group.getUsers();
    users.add(user);

    List<QFAGroup> groups = new ArrayList<QFAGroup>();
    groups.add(group);
  ...
  private void createUserAttributesFromLdap(QFAUser user, Map<String, List<String>> ldap) {

    for (Entry<String, String> entry : attributesMap.entrySet()) {
      String aaaAttr = entry.getKey();
      String ldapAttr = entry.getValue();

      QFAUserAttribute attribute = new QFAUserAttribute();
  ...
  private String updateUserFromLdap(QFAUser user, Map<String, List<String>> ldap) {
    String userId = user.getId();
  ...
  private String updateGroupFromLdap(QFAUser user, Map<String, List<String>> ldap) {
    QFAGroup oldGroup = user.getGroups().get(0);
    ...
    QFAGroup newGroup = em.find(QFAGroup.class, newGroupId);
    if (newGroup == null) {
      return null;
    }
  ...
  private void updateUserAttributesFromLdap(QFAUser user, Map<String, List<String>> ldap) {
    List<QFAUserAttribute> attributes = user.getUserAttributes();

    for (Entry<String, String> entry : attributesMap.entrySet()) {
      String aaaAttr = entry.getKey();
      String ldapAttr = entry.getValue();

      QFAUserAttribute attribute = findByName(attributes, aaaAttr);
      if (attribute == null) {
        continue;
      }
  ...
  private QFAUserAttribute findByName(List<QFAUserAttribute> attributes, String name) {
    for (QFAUserAttribute attribute : attributes) {
