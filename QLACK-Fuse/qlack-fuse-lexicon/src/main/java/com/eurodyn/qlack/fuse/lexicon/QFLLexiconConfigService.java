...
import com.eurodyn.qlack.fuse.lexicon.exception.QFLLexiconYMLProcessingException;
...
@Validated
@Transactional
public class QFLLexiconConfigService {

  private static final Logger LOGGER = Logger.getLogger(QFLLexiconConfigService.class.getName());

  @PersistenceContext
  private EntityManager em;

  // Service references.
  private QFLGroupService groupService;
  private QFLLanguageService languageService;
  private QFLKeyService keyService;

  @Autowired
  public QFLLexiconConfigService(QFLGroupService groupService,
      QFLLanguageService languageService, QFLKeyService keyService) {
  ...
  private void updateKeys(List<Map<String, Object>> translations, String groupId,
    ...
    for (Map<String, Object> translation : translations) {
      ...
      QFLKeyDTO keyDTO = keyService.getKeyByName(translationKey, groupId, true);
      // If the key does not exist in the DB then create it.
      if (keyDTO == null) {
        keyDTO = new QFLKeyDTO();
    ...
    try {
      ...
      if (groups != null) {
        for (Map<String, Object> group : groups) {
          ...
          QFLGroupDTO groupDTO = groupService.getGroupByName(groupName);
          // If a group with this name does not exist create it.
          if (groupDTO == null) {
            groupDTO = new QFLGroupDTO();
      ...
      if (languages != null) {
        for (Map<String, Object> language : languages) {
          ...
          QFLLanguageDTO languageDTO = languageService.getLanguageByLocale(locale);
          // If a language with this locale does not exist create it.
          if (languageDTO == null) {
            languageDTO = new QFLLanguageDTO();
      ...
      if (translationContents != null) {
        for (Map<String, Object> translationContent : translationContents) {
          ...
          if (excludedGroupName != null) {
            Set<QFLGroupDTO> newGroups = groupService.getRemainingGroups(excludedGroupName);
            for (QFLGroupDTO group : newGroups) {
      ...
      throw new QFLLexiconYMLProcessingException("Error handling lexicon YAML file");
    }
